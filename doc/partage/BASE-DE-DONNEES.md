...
### `chapters/{chapterId}/fiche/main` 🟡

Fiche de révision condensée d'un chapitre — optionnelle. Rendue côté mobile via `PedagogicalContent` (même rendu que `lessons/{id}/content/main`).

```typescript
interface ChapterFicheDoc {
  fr: string;   // Markdown complet FR (blocs :::definition, :::retenir, etc.)
  en: string;   // Markdown complet EN
}
```

- Lecture : `.get()` (statique, cache Firestore offline actif). Pas de `snapshots()`.
- 1 read par session si l'onglet "Fiche" est ouvert. La plupart des chapitres n'ont pas de fiche → `doc.exists == false` → état vide côté mobile.
- Pas de nouvel index Firestore (lecture par chemin direct `chapters/{id}/fiche/main`).
- Seed : `seed_3e_content.py` écrit `chapters/{id}/fiche/main` si le champ `fiche` est présent dans le JSON source.
- ⚠️ **Accord backend requis** avant toute modification du schéma (Story 2.5).
